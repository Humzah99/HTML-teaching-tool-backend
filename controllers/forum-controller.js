...
const getForumQuestionById = async(req, res, next) => {
    ...
    res.json({
        forumQuestion: forumQuestion.toObject({
            getters: true
        })
    });
};
...
const getForumQuestionByUserId = async(req, res, next) => {
    ...
    res.json({
        forumQuestions: forumQuestions.map(forumQuestion => forumQuestion.toObject({
            getters: true
        }))
    });
};
...
const updateQuestion = async(req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            'Something went wrong, could not update question.', 500
        );
        return next(error);
    }
    ...
    res.status(200).json({
        question: forumQuestion.toObject({
            getters: true
        })
    });
};
const deleteQuestion = async(req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            'Something went wrong, could not delete question.', 500
        );
        return next(error);
    }

    try {
        await forumQuestion.remove();
    } catch (err) {
        const error = new HttpError(
            'Something went wrong, could not delete question.', 500
        );
